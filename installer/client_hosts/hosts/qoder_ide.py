"""Independent macOS Qoder IDE host declaration."""
...
def _app_root() -> Path:
    return qoder_ide_common.configured_path(
        "QODER_IDE_APP_ROOT", Path("/Applications/Qoder IDE.app")
    )


def _config_path() -> Path:
    return qoder_ide_common.configured_path(
        "QODER_IDE_CONFIG", Path.home() / ".qoder" / "mcp.json"
    )


def _settings_path() -> Path:
    return qoder_ide_common.configured_path(
        "QODER_IDE_SETTINGS", Path.home() / ".qoder" / "settings.json"
    )


def _skills_path() -> Path:
    return qoder_ide_common.configured_path(
        "QODER_IDE_SKILLS_DIR", Path.home() / ".qoder" / "skills"
    )
...
def _product_metadata():
    return qoder_ide_common.product_metadata(
        _app_root(),
        application_name="qoder",
        bundle_identifier="com.qoder.ide",
    )
...
def _skills_configured() -> bool:
    ...
    try:
        return mcp_config.read_entry("qoder-ide") is not None
    except ShellError:
        return False
...
QODER_IDE = AgentHostSpec(
    id="qoder-ide",
    transport="stdio",
    launch_policy="absolute-bootstrap-v1",
    config_format="json",
    host_family="qoder-ide",
    config_env="QODER_IDE_CONFIG",
    ...
    observed_client_aliases=frozenset({"qoder"}),
    ...
    skill_route_name="qoder-ide",
    repair_skills_on_setup=True,
    skills_require_managed_target=True,
    client_name_patterns=(r"^\s*qoder\s+ide\s*$",),
