...
def _https_context() -> ssl.SSLContext:
    """Resolve the TLS trust store for hub fetches: explicit override, else the platform store,
    with certifi ONLY as a last-resort rescue when the platform ships no usable store.

    The platform default store is authoritative and is never widened on a machine that has one.
    `create_default_context()` uses it, so an OS / enterprise decision to distrust a CA
    (revocation, compliance policy) is honored. The prior code seeded the context from
    `certifi.where()` EXCLUSIVELY, which dropped the OS store — on Windows that store carries
    roots a stale/partial certifi can lack (the GTS chain behind the hub is one), so the MCP
    launcher died with CERTIFICATE_VERIFY_FAILED. certifi is now loaded only when the platform
    has no store at all (a minimal container, or a Python build shipping no default CA file),
    where it is the sole thing that lets verification succeed. It never sits on top of a
    populated OS store, so it can never re-trust a root the OS deliberately distrusts.

    Intentionally duplicated byte-for-byte in `installer.activate._https_context` and
    `client.runner.https_context`. The managed launcher (installer/launcher.py
    `_load_candidate_shim`) imports this shim through a fail-closed integrity gate that rejects
    any import target outside the managed installation; giving the shim a cross-module import to
    a shared helper would widen that trust boundary. The three copies are kept from drifting by
    installer/tests/test_https_context_trust_store.py, which runs the contract against all three.
    """
    ...
    try:
        ...
        _log("certifi trust-store rescue unavailable; TLS may fail closed")
    return context
