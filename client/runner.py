...
def https_context() -> ssl.SSLContext:
    """Resolve the TLS trust store for hub fetches (SSL_CERT_FILE / REQUESTS_CA_BUNDLE, else the
    platform store, with certifi ONLY as a last-resort rescue for a platform that ships none).

    Kept byte-for-byte equivalent to `installer.shim._https_context`; see that docstring for why
    certifi is a fallback rather than a supplement (it must never re-trust a root the OS
    distrusts) and why the three copies are duplicated on purpose.
    """
    ...
    try:
        ...
        print("de-runner: certifi trust-store rescue unavailable; TLS may fail closed", file=sys.stderr)
    return context
